...
if defined AUDION_DISABLE_FZF goto FALLBACK_MENU
...
>>"%MENU_FILE%" echo [73] SMOKE TESTS                           ^| smoke_tests       ^| run project smoke tests
...
if /I "%RAW%"=="smoke_tests" goto SMOKE_TESTS
...
:SMOKE_TESTS
if not exist "%BASE_DIR%\runtime\python.exe" (
  echo [ERROR] runtime\python.exe was not found.
  if not defined AUDION_NO_PAUSE pause
  goto MAIN
)
if not exist "%BASE_DIR%\tests\smoke.py" (
  echo [ERROR] tests\smoke.py was not found.
  if not defined AUDION_NO_PAUSE pause
  goto MAIN
)
"%BASE_DIR%\runtime\python.exe" "%BASE_DIR%\tests\smoke.py" --quick
echo.
if not defined AUDION_NO_PAUSE pause
goto MAIN
